...
let styles = lazypipe()
    ...
    .pipe(plugins.autoprefixer, {browsers: ['last 1 version']}) //适合各种浏览器
    //.pipe(plugins.uncss,{html: [paths.client.mainView, `${clientPath}/{app,components}/**/*.html`]}) //删除未使用
...
gulp.task('watch', () => {
    ...
    plugins.livereload.listen({port:35712});
    ...
    plugins.watch(paths.client.styles, () => {  //['inject:scss']
        gulp.src(paths.client.mainStyle)
            ...
            .pipe(styles())
            .pipe(gulp.dest('.tmp/app'))
            .pipe(plugins.livereload({port:35712}));
    });
    ...
    plugins.watch(paths.client.scripts,{verbose:true,readDelay:100}) //['inject:js']
        .pipe(plugins.plumber())
        .pipe(transpileClient())
        .pipe(gulp.dest('.tmp'))
        .pipe(plugins.livereload({port:35712}));
...
gulp.task('serve', cb => {
    runSequence(
        ['env:dev','clean:tmp', 'constant', 'env:all'],
        ['lint:scripts', 'inject'],
        ['wiredep:client'],
...
gulp.task('build', cb => {
  runSequence(
    ...
    'env:prod',
    'inject', //插入自定义js css scss
    'wiredep:client', //插入依赖
    [
      'transpile:client', //es6 转换
      'transpile:server' //es6 转换
    ],
    [
      //'build:images', //图片手动cdn 不用处理
      'copy:extras', //拷贝附加内容
      'copy:fonts', //拷贝字体
      'copy:assets', //拷贝资产
      'copy:server', //拷贝服务器
...
gulp.task('build:client', [
  ...
  //'build:images' //构建图片
    ...
    return gulp.src(paths.client.mainView)
                .pipe(plugins.useref()) //合并压缩html文件中的文件
                .pipe(appFilter)
                  .pipe(plugins.addSrc.append('.tmp/templates.js'))
                  .pipe(plugins.concat({path:'app/app.js',cwd:''}))
                .pipe(appFilter.restore)
                .pipe(jsFilter)
                  .pipe(plugins.ngAnnotate()) //插入AngularJS依赖
                  .pipe(plugins.uglify())
...
gulp.task('html', function() {
    return gulp.src(`${clientPath}/{app,components}/**/*.html`)
        ...
        .pipe(plugins.angularTemplatecache({module: 'clientApp'}))
        .pipe(gulp.dest('.tmp'));
});
...
gulp.task('constant', function() {
  ...
  let appConfig = sharedConfig[env];
  return plugins.ngConstant({
    name: 'clientApp.constants',
    deps: [],
    wrap: true,
    stream: true,
...
gulp.task('oss', function(){
    return gulp.src([
        './dist/client/app/*',
        //'./dist/client/bower_components/**/fonts/*'
      ], { base: 'dist/client' })
      .pipe(plugins.alioss({
